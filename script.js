/**
 * script.js — AI Student Impact
 * Calls /api/predict on the same Vercel domain — no separate backend needed.
 */
...
function updateFill(el) {
  ...
  el.style.background =
    `linear-gradient(to right, #6c63ff ${pct}%, #2d3148 ${pct}%)`;
}
...
predictBtn.addEventListener("click", async () => {
  ...
  try {
    // Relative URL — works locally (if served) and on Vercel automatically
    ...
    showError("Network error — make sure the app is deployed or run locally with `vercel dev`.");
  } finally {
    setLoading(false);
  }
});
...
function displayResult({ predicted_post_gpa, grade_letter, insight }) {
  ...
  setTimeout(() => { document.getElementById("gpaBar").style.width = `${pct}%`; }, 50);
